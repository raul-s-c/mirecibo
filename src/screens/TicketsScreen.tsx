...
export function TicketsScreen({ onScan, onManual }: { onScan: () => void; onManual: () => void }) {
  ...
  return <div className="screen">
    ...
    {!state.receipts.length ? <EmptyState icon={<ReceiptText />} title="Todavía no hay tickets" text="Fotografía uno o registra el gasto a mano para conservarlo." action={<Button onClick={onManual}>Crear el primero</Button>} /> : search.trim() ? (!productResults.length ? <EmptyState icon={<Search />} title="No aparece en tus tickets" text="Prueba con una parte del nombre. La búsqueda ignora mayúsculas y acentos." /> : <div className="ticket-product-results">{productResults.map(({ receipt, line }) => <button key={`${receipt.id}-${line.id}`} onClick={() => setSelected(receipt)}><span className="product-search-icon"><Search /></span><span><b>{line.name}</b><small>Ticket #{ticketNumbers.get(receipt.id)} · {receipt.store}</small><small>{shortDate(receipt.date)} · {line.quantity.toLocaleString('es-ES')} {line.unit} · {money(line.unitPrice || line.total / Math.max(line.quantity, 1))}/{line.unit}</small></span><strong className={line.total < 0 ? 'credit-amount' : undefined}><small>Importe</small>{money(line.total)}</strong><ChevronRight /></button>)}</div>) : <div className="ticket-list">{state.receipts.map(receipt => <button key={receipt.id} onClick={() => setSelected(receipt)}><span className="store-mark">{receipt.store.slice(0, 1).toLocaleUpperCase('es')}</span><span><b>{receipt.store}</b><small>{shortDate(receipt.date)}{receipt.time ? ` · ${receipt.time}` : ''} · {receipt.total < 0 ? 'Abono' : `${receipt.lines.length} ${receipt.lines.length === 1 ? 'concepto' : 'productos'}`}</small></span><strong className={receipt.total < 0 ? 'credit-amount' : undefined}>{money(receipt.total)}</strong><ChevronRight size={19} /></button>)}</div>}
    <Sheet open={Boolean(selected)} title={editing ? 'Editar ticket' : 'Detalle del ticket'} onClose={close} wide>{selected && editing ? <ReceiptEditor receipt={selected} categoryNames={activeCategories(state).map(value => value.name)} onCancel={() => setEditing(false)} onSave={receipt => { updateReceipt(receipt); setSelected(receipt); setEditing(false); }} /> : selected ? <div className="receipt-detail">
      <div className="receipt-summary"><span className="store-mark large">{selected.store.slice(0, 1)}</span><div><h3>{selected.store}</h3><p>{shortDate(selected.date)} {selected.time ?? ''}{selected.total < 0 ? ' · Abono' : ''}</p></div><strong className={selected.total < 0 ? 'credit-amount' : undefined}>{money(selected.total)}</strong></div>
      <div className="receipt-table"><div className="receipt-table__head"><span>Producto</span><span>Cant.</span><span>P. unit.</span><span>Importe</span></div>{selected.lines.map(line => <div key={line.id}><span><b>{line.name}</b><small>{line.category}</small></span><span>{line.quantity.toLocaleString('es-ES')} {line.unit}</span><span className={`receipt-unit-price ${line.unitPrice < 0 ? 'credit-amount' : ''}`}>{money(line.unitPrice || line.total / Math.max(line.quantity, 1))}<small>/{line.unit}</small></span><strong className={line.total < 0 ? 'credit-amount' : undefined}>{money(line.total)}</strong></div>)}</div>
      <div className="receipt-total"><span>Total</span><strong className={selected.total < 0 ? 'credit-amount' : undefined}>{money(selected.total)}</strong></div>
